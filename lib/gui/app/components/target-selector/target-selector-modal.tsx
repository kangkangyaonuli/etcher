...
> {
	...
	constructor(props: Readonly<TargetSelectorModalProps>) {
		super(props);
		...
		const enrichedDrivesData = _.map(drives, (drive) => {
			return {
				...
				disabled: !isDriveValid(drive, this.image) || drive.progress,
			};
		});
		...
		this.state = {
			...
			disabledRows: drives
				.filter((drive) => !isDriveValid(drive, this.image) || drive.progress)
